...
namespace Modbus.ModbusFunctions
{
    ...
    public class ReadInputRegistersFunction : ModbusFunction
    {
        ...
        public override byte[] PackRequest()
        {
            ...
            Buffer.BlockCopy(
                BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)parameters.TransactionId)),
                0, request, 0, 2);

            Buffer.BlockCopy(
                BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)parameters.ProtocolId)),
                0, request, 2, 2);

            Buffer.BlockCopy(
                BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)parameters.Length)),
                0, request, 4, 2);
            ...
            Buffer.BlockCopy(
                BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)parameters.StartAddress)),
                0, request, 8, 2);

            Buffer.BlockCopy(
                BitConverter.GetBytes(IPAddress.HostToNetworkOrder((short)parameters.Quantity)),
                0, request, 10, 2);
        ...
        public override Dictionary<Tuple<PointType, ushort>, ushort> ParseResponse(byte[] response)
        {
            ...
            for (int i = 0; i < byteCount / 2; i++)
            {
                ...
                byte secondByte = response[9 + i * 2 + 1];

                ushort value = BitConverter.ToUInt16(new byte[2] { secondByte, firstByte }, 0);

                dict.Add(new Tuple<PointType, ushort>(PointType.ANALOG_INPUT, startAddress++), value);
            }
